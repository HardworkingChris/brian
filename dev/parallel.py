...
class DefaultNetwork(Network):
    def __init__(self, p):
        ...
        # chain 1
        print "Total neurons in chain 1 = ",p.total_neurons,"\n"
        ...
        # chain 2
        print "Total neurons in chain 2 = ",p.total_neurons,"\n"
        
        unscaled_E2 = int(p.neurons_per_layer * 0.88)
        print "nE unscaled = ",unscaled_E,"\n"
        
        scaled_E2 = int(p.neurons_per_layer * p.neuron_multiply * 0.88)
        print "nE scaled = ",scaled_E,"\n"
        
        unscaled_I2 = int(p.neurons_per_layer * 0.12)
        print "nI unscaled = ",unscaled_I,"\n"
        
        scaled_I2 = int(p.neurons_per_layer * p.neuron_multiply * 0.12)
        print "nI scaled = ",scaled_I,"\n"
        
        layer_E2 = [ chaingroup.subgroup(unscaled_E) if i < (p.num_layers-1) else chaingroup.subgroup(scaled_E) for i in range(p.num_layers) ]
        
        layer_I2 = [ chaingroup.subgroup(unscaled_I) if i < (p.num_layers-1) else chaingroup.subgroup(scaled_I) for i in range(p.num_layers) ]
        ...
        for i in range(p.num_layers - 1):            
            if i == 4 and p.num_layers:
                chainconnect.connect_random(layer_E[i], layer_E2[i], sparseness = p.pr, weight = p.psp_peak * p.we)
                chainconnect.connect_random(layer_I[i], layer_I2[i], sparseness = p.pr, weight = p.psp_peak * p.wi)         
                chainconnect.connect_random(layer_I[i], layer_E2[i], sparseness = p.pr, weight = p.psp_peak * p.wi)
                chainconnect.connect_random(layer_E[i], layer_I2[i], sparseness = p.pr, weight = p.psp_peak * p.we)
            else:
                ...
                chainconnect.connect_random(layer_E2[i], layer_E2[i + 1], sparseness = p.pr, weight = p.psp_peak * p.we)    
                chainconnect.connect_random(layer_E2[i], layer_I2[i + 1], sparseness = p.pr, weight = p.psp_peak * p.we)
                chainconnect.connect_random(layer_I2[i], layer_E2[i + 1], sparseness = p.pr, weight = p.psp_peak * p.wi)
                chainconnect.connect_random(layer_I2[i], layer_I2[i + 1], sparseness = p.pr, weight = p.psp_peak * p.wi)     
        ...
        inputconnect_E2 = Connection(inputgroup, layer_E2[0], 2)
        inputconnect_E2.connect_full(weight = p.psp_peak * p.we)
        inputconnect_I2 = Connection(inputgroup, layer_I2[0], 2)
        inputconnect_I2.connect_full(weight = p.psp_peak * p.we)
        
        # monitors
        chainmon_E = [SpikeMonitor(g, True) for g in layer_E]
        chainmon_I = [SpikeMonitor(g, True) for g in layer_I]
        chainmon_E2 = [SpikeMonitor(g, True) for g in layer_E2]
        chainmon_I2 = [SpikeMonitor(g, True) for g in layer_I2]

        inputmon = SpikeMonitor(inputgroup, True)
        mon_E = [inputmon] + chainmon_E
        mon_I = [inputmon] + chainmon_I
        mon_E2 = [inputmon] + chainmon_E2
        mon_I2 = [inputmon] + chainmon_I2

        # network
        Network.__init__(self, chaingroup, inputgroup, chainconnect, inputconnect_E, inputconnect_I, mon_E, mon_I, mon_E2, mon_I2)
        ...
        self.layer_E2 = layer_E2
        self.layer_I2 = layer_I2
        self.params = p
