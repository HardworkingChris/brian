...
P = NeuronGroup(N=1500, model=eqs,
...
'''
user_n2 = int(raw_input("\na for chain 2 = "))
user_sigma2 = float(raw_input("sigma(in mSec) for chain 2 = "))
'''
print "\n"
Pinput = PulsePacket(t=10 * ms, n=user_n, sigma=user_sigma * ms)
#Pinput2 = PulsePacket(t=10 * ms, n=user_n2, sigma=user_sigma2 * ms)
...
Pgp_rnd = [ P.subgroup(50) for i in range(10)]
'''
...
for i in range(9):
    ...
    C.connect_full(Pgp_rnd[i], Pgp_E[i + 1], weight_E * 0.01)
    C.connect_full(Pgp_rnd[i], Pgp_E[i + 1], weight_E * 0.01)
